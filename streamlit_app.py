...
st.sidebar.markdown(
    ...
    <div style="position: fixed; bottom: 0; width: 100%; text-align: left;">
        ...
        <p>Sahasra Koppal  </p>
        <p>Shreya Reddy Chidirala </p>
         <p>Yoshitha Reddy Lingaladinne </p>
    </div>
    """,
...
def dashboard():
    ...
    #     }
        ...
        # Netflix Barrier Chart
    df['Netflix Barrier'] = df['Netflix Barrier'].replace('N/A', 'No Issues')

    barrier_counts = df['Netflix Barrier'].value_counts().reset_index()
    ...
    fig5 = px.bar(
        ...
        x='Barrier',
        y='Count',
        labels={'Barrier': 'Factors', 'Count': 'Count'},
        color='Count',
        color_continuous_scale='Reds'
    )
    ...
    col4.plotly_chart(fig5, key=f"fig5_barrier_{uuid.uuid4()}")
    ...
    col4.plotly_chart(fig5, key="fig5_barrier")
    col4.markdown(
    """
    <div style="text-align: center;">
        The bar chart shows that the high subscription cost is the most significant factor affecting Netflix usage, followed by the lack of desired shows and movies.
    ...
    genre_counts = df['Favorite Genre'].value_counts().reset_index()
    ...
    fig6 = px.bar(
        ...
        y='Count',
        color='Count',
        labels={'Genre': 'Genres', 'Count': 'Count'},
        color_continuous_scale='Reds'
    )
